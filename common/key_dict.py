...
keys = dict(NAME=[get_name, "随机生成姓名 用法示例：$NAME()$"], PHONE=[get_phone_number, "随机生成手机号 用法示例：$PHONE()$"],
            FULL_ADDRESS=[get_full_address, "随机生成全量地址 用法示例：$FULL_ADDRESS()$"],
            STREET_ADDRESS=[get_detail_address, "随机生成街道地址 用法示例：$STREET_ADDRESS()$"],
            EMAIL=[get_email, "随机生成邮箱地址 用法示例：$EMAIL()$"], PASSWORD=[get_password, "随机生成密码 用法示例：$PASSWORD()$"],
            TEXT=[get_text, "随机生成一段文本 用法示例：$TEXT()$"], COLOR=[get_color, "随机生成颜色（英文） 用法示例：$COLOR()$"],
            ...
            IPV6=[get_ipv6, "随机生成ipv6地址 用法示例：$IPV6()$"], IPV4=[get_ipv4_public, "随机生成ipv4地址 用法示例：$IPV64()$"],
