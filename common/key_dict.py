...
keys = dict(NAME=[get_name, "随机生成姓名 用法示例：$NAME()$"],
            ...
            IPV4=[get_ipv4_public, "随机生成ipv4地址 用法示例：$IPV4()$"],
            CHROME=[get_chrome, "随机生成chrome User-Agent 用法示例：$CHROME()$"],
            RANDOM_STRING=[get_string, "随机生成指定长度字符串 用法示例：$RANDOM_STRING(字符范围,字符串长度)$"],
            RANDOM_LENGTH_STRING=[get_random_length_string,
                                  "随机生成长度在指定范围内随机的字符串 用法示例：$RANDOM_LENGTH_STRING(字符范围,最小长度,最大长度)$"],
            RANDOM_INT=[get_random_int, "随机生成整数 用法示例：$RANDOM_INT(最小值,最大值)$"],
            SET_VAR=[set_var, "设置变量的值 用法示例：$SET_VAR(变量名,变量值)$"],
            GET_VAR=[get_var, "获取变量的值 用法示例：$GET_VAR(变量名)$"],
            JSON_EXTRACTOR=[json_extractor, "json提取器，根据jsonpath提取响应正文中的数据 用法示例：$JSON_EXTRACTOR(变量名,json_path,0)$"],
            ASSERT_CONTAINS=[assert_contains, "断言响应正文中是否包含expect 用法示例：$ASSERT_CONTAINS(预期结果)$"],
            ASSERT_JSON=[assert_json, "断言响应json中json_path对应字段的值和expect的值是否相等 用法示例：$ASSERT_JSON(json_path,预期结果)$"],
            MYSQL=[msyql_db, "执行sql语句，并把执行结果存入变量var_name中 用法示例：$MYSQL(变量名,子系统代号,sql语句)$"],
            MD5=[md5, "md5加密,返回加密后的数据 用法示例：$MD5(待加密数据)$"]
            )
